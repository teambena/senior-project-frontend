...
export const customer = {
	...
	mutations: {
		...
		updateRecord(state, record) {
			const item = state.records.find(item => item['customer_id'] === record['customer_id']);
			if(item){
		...
		deleteRecord(state, id) {
			if(Array.isArray(id)){
				id.forEach((itemId) => {
					let itemIndex = state.records.findIndex(item => item['customer_id'] == itemId);
					if(itemIndex!=-1){
				...
				let itemIndex = state.records.findIndex(item => item['customer_id'] == id);
				if(itemIndex!=-1){
					state.records.splice(itemIndex, 1);
				}
			}
