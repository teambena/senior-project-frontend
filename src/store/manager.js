...
export const manager = {
	...
	mutations: {
		...
		updateRecord(state, record) {
			const item = state.records.find(item => item['manager_id'] === record['manager_id']);
			if(item){
		...
		deleteRecord(state, id) {
			if(Array.isArray(id)){
				id.forEach((itemId) => {
					let itemIndex = state.records.findIndex(item => item['manager_id'] == itemId);
					if(itemIndex!=-1){
				...
				let itemIndex = state.records.findIndex(item => item['manager_id'] == id);
				if(itemIndex!=-1){
					state.records.splice(itemIndex, 1);
				}
			}
