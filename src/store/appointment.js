...
export const appointment = {
	...
	mutations: {
		...
		updateRecord(state, record) {
			const item = state.records.find(item => item['appointment_id'] === record['appointment_id']);
			if(item){
		...
		deleteRecord(state, id) {
			if(Array.isArray(id)){
				id.forEach((itemId) => {
					let itemIndex = state.records.findIndex(item => item['appointment_id'] == itemId);
					if(itemIndex!=-1){
				...
				let itemIndex = state.records.findIndex(item => item['appointment_id'] == id);
				if(itemIndex!=-1){
					state.records.splice(itemIndex, 1);
				}
			}
